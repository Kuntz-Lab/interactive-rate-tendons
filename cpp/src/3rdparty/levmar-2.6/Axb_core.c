...
#define AX_EQ_B_SVD_INIT LM_ADD_PREFIX(Ax_eq_b_SVD_init) // common code
#define AX_EQ_B_SVD LM_ADD_PREFIX(Ax_eq_b_SVD)
#define AX_EQ_B_SVD_SING LM_ADD_PREFIX(Ax_eq_b_SVD_sing)
...
/*
 * Common initialization for both AX_EQ_B_SVD* functions
 *
 * Extracted by Michael Bentley from AX_EQ_B_SVD when writing AX_EQ_B_SVD_SING.
 *
 * This checks inputs and makes sure the given buffer is allocated to the right
 * size.
 *
 * Returns 1 on failure and 0 on success
 */
int AX_EQ_B_SVD_INIT(LM_REAL *A, LM_REAL *B, LM_REAL *x, int m, int n,
                     LM_REAL **buf, int *buf_sz, int *work_sz)
{
int iwork_sz, tot_sz, info, maxdim, mindim, a_sz, u_sz, s_sz, vt_sz;
LM_REAL thresh;
char svd_type[2];
  ...
  svd_type[0]=(m==n ? 'A' : 'S');
  svd_type[1]='\0';
  *work_sz=-1; // workspace query. Keep in mind that GESDD requires more memory than GESVD
  maxdim=(m>n ? m : n);
  mindim=(m>n ? n : m);
  /* note that optimal work size is returned in thresh */
  GESVD(svd_type, svd_type, (int *)&m, (int *)&n, NULL, (int *)&m, NULL, NULL, (int *)&m, NULL, (int *)&mindim, (LM_REAL *)&thresh, work_sz, &info);
  //GESDD("A", (int *)&m, (int *)&m, NULL, (int *)&m, NULL, NULL, (int *)&m, NULL, (int *)&m, (LM_REAL *)&thresh, work_sz, NULL, &info);
  *work_sz=(int)thresh;
#else /* use minimum size */
  *work_sz=5*m; // min worksize for GESVD
  //*work_sz=m*(7*m+4); // min worksize for GESDD
#endif
  iwork_sz=8*m;
  a_sz=m*n;
  u_sz=m*mindim;
  ...
  tot_sz=(a_sz + u_sz + s_sz + vt_sz + *work_sz)*sizeof(LM_REAL) + iwork_sz*sizeof(int); /* should be arranged in that order for proper doubles alignment */
  ...
  if(tot_sz>*buf_sz){ /* insufficient memory, allocate a "big" memory chunk at once */
    ...
    *buf=(LM_REAL *)malloc(tot_sz);
    if(!*buf){
      fprintf(stderr, RCAT("memory allocation in ", AX_EQ_B_SVD_INIT) "() failed!\n");
      exit(1);
    }
  }
#else
...
int a_sz, u_sz, s_sz, vt_sz;
LM_REAL thresh, one_over_denom;
register LM_REAL sum;
int info, rank, worksz, *iwork, iworksz;

  if(0 != AX_EQ_B_SVD_INIT(A, B, x, m, m, &buf, &buf_sz, &worksz)) {
    return 1;
  }
 ...
 * This function returns the solution of Ax = b
 *
 * This copy was created by Michael Bentley from AX_EQ_B_SVD and modified to
 * handle small singular values.  It is also modified to handle general
 * non-square matrix A.
 *
 * The function is based on SVD decomposition:
 * If A=U D V^T with U, V orthogonal and D diagonal, the linear system becomes
 ...
 * Note that V D^{-1} U^T is the pseudoinverse A^+
 *
 * This variant uses the SVD decomposition to remove the influence of zero and
 * very small singular values in the solution.  The solution is:
 *   x = V D^{-1} U^T b = sum_i^n (u_i^T b v_i / d_i)
 * Singular values are the d_i.  For ones that are zero or very small, we can
 * simply skip those terms in the sum completely.  Otherwise, dividing by a
 * very small number, causes that one singular vector v_i to have a very large
 * influence on x and induces large error.
 *    (see equation 10.22 in "Numerical Optimization" by Nocedal and Wright)
 *
 * x is nx1, A is mxn, and b is mx1.
 *   - U is mxm, D is mxn, V is nxn
 *
 * The function returns 0 in case of error, 1 if successful
 *
 * This function is often called repetitively to solve problems of identical
 ...
 * A call with NULL as the first argument forces this memory to be released.
 */
int AX_EQ_B_SVD_SING(LM_REAL *A, LM_REAL *B, LM_REAL *x, int m, int n)
{
__STATIC__ LM_REAL *buf=NULL;
__STATIC__ int buf_sz=0;
static LM_REAL eps=LM_CNST(-1.0);
...
register int i, j;
LM_REAL *a, *u, *s, *vt, *work;
int maxdim, mindim, a_sz, u_sz, s_sz, vt_sz;
LM_REAL thresh, one_over_denom;
register LM_REAL sum;
int info, rank, worksz, *iwork, iworksz;
   
  if(0 != AX_EQ_B_SVD_INIT(A, B, x, m, n, &buf, &buf_sz, &worksz)) {
    return 1;
  }
  ...
  /* SVD decomposition of A */
  GESVD("S", "S", (int *)&m, (int *)&n, a, (int *)&m, s, u, (int *)&m, vt, (int *)&mindim, work, (int *)&worksz, &info);

  /* error treatment */
  if(info!=0){
    if(info<0){
      fprintf(stderr, RCAT(RCAT(RCAT("LAPACK error: illegal value for argument %d of ", GESVD), "/" GESDD) " in ", AX_EQ_B_SVD_SING) "()\n", -info);
      exit(1);
    }
    else{
      fprintf(stderr, RCAT("LAPACK error: dgesdd (dbdsdc)/dgesvd (dbdsqr) failed to converge in ", AX_EQ_B_SVD_SING) "() [info=%d]\n", info);
#ifndef LINSOLVERS_RETAIN_MEMORY
      free(buf);
#endif
  ...
  // TODO[Mike B]: make a variant that checks for singular values and accounts for them
  // TODO[Mike B]: using equation 10.22 in "Numerical Optimization" by Nocedal

  // compute x = sum_i^n (u_i^T b v_i / d_i), skipping terms where d_i is small
  // - u_i is the ith column of u  (m-by-min(n,m))
  // - v_i is the ith row of vt    (min(m,n)-by-n)
  // - d_i is the ith element of s
  // - d_i is small if it is less than 1e-6 times the largest one d_0
  // note: u and vt are matrices in column-major format
  // note: when interpretting vt as row-major, it is V (not transposed)

  // first, compute vector of coefficients s_i <- u_i^T b / s_i
  // this reuses the s vector from singular values to coefficients of v_i
  for(rank=0, thresh=1e-8*s[0]; rank<mindim && s[rank]>thresh; rank++){
    for(j=0, sum=0.0; j<m; j++) {
      sum+=u[j+rank*mindim]*B[j];
    }
    s[rank]=sum/s[rank];
  }

  // then, x = V s (but only until rank)
  // note: we sum from the largest singular value to the smallest for
  // floating-point accuracy.  If you sum floats from smallest to largest, you
  // can mitigate some error.
  for(i=0; i<n; i++){
    for(j=0, sum=0.0; j<rank; j++){ // rank <= mindim
      sum+=vt[i*mindim+j]*s[j];
    }
    x[i]=sum;
  }
...
#undef AX_EQ_B_LU
#undef AX_EQ_B_SVD
#undef AX_EQ_B_SVD_SING
#undef AX_EQ_B_SVD_INIT
#undef AX_EQ_B_BK
