  ...
  printf("-- errors %.9g %0.9g, #active %d [%d]\n", jacTe_inf, p_eL2, numactive, j);
   ...
   /* compute initial damping factor */
    ...
    while(1){
      ...
      for(i=0; i<m; ++i){
        jacTjac[i*m+i]+=mu;      // J^T J + mu*I
        //jacTjac[i*m+i]*=(1+mu);  // J^T J + mu*diag(J^T J)
      }
      ...
      //issolved=AX_EQ_B_SVD_SING(jacTjac, jacTe, Dp, m, m); ++nlss; linsolver=(int (*)(LM_REAL *A, LM_REAL *B, LM_REAL *x, int m))AX_EQ_B_SVD_SING;
#else
